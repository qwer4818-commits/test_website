...
- **Remaining limitations.** Lots of ties — food/drink lack cafe-style tag bonuses, so several places land on the same score and the JSON array order becomes the tiebreak (close to arbitrary). Reviews aren't shared because they're localStorage. Distance is a self-selected category rather than a computed value — by design for a no-backend demo, with location-based real distance left as a future step. The data is hand-curated, so it can drift from real business status.
